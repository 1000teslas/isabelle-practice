...
lemma dfs_spec: "\<Gamma> \<turnstile> \<lbrace> \<acute>root \<in> verts \<acute>G \<and> fin_digraph \<acute>G \<rbrace> dfs \<lbrace> \<acute>discovered = {v. reachable \<acute>G \<acute>root v} \<rbrace>"
