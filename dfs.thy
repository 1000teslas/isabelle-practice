...
"dfs \<equiv>
  ...
  INV \<lbrace> fin_digraph \<acute>G
...
lemma dfs_partial: "\<Gamma> \<turnstile> \<lbrace> \<acute>root \<in> verts \<acute>G \<and> fin_digraph \<acute>G \<rbrace> dfs \<lbrace> \<acute>discovered = {v. reachable \<acute>G \<acute>root v} \<rbrace>"
...
lemma dfs_total: "\<Gamma> \<turnstile>\<^sub>t \<lbrace> \<acute>root \<in> verts \<acute>G \<and> fin_digraph \<acute>G \<rbrace> dfs \<lbrace> \<acute>discovered = {v. reachable \<acute>G \<acute>root v} \<rbrace>"
  unfolding dfs_def apply (vcg; clarsimp simp add: fin_digraph.axioms(1) wf_digraph.reachable_refl)
  apply (subgoal_tac "set (SOME ws. set ws = {pre_digraph.head G e |e. e \<in> arcs G \<and> tail G e = hd stack})
    = {pre_digraph.head G e |e. e \<in> arcs G \<and> tail G e = hd stack}")
   apply clarsimp
